...
import { deleteRolePickerPanel } from '@/actions/role-picker';
...
interface DeleteRolePickerButtonProps {
  readonly guildId: string;
  readonly panelId: string;
}

export function DeleteRolePickerButton({
  guildId,
  panelId,
}: DeleteRolePickerButtonProps): React.JSX.Element {
  ...
  async function handleConfirm(): Promise<void> {
    ...
    setSubmitting(true);
    const result = await deleteRolePickerPanel({ guildId, panelId });
    if (!result.ok) {
      toast.error(result.error.message);
      setSubmitting(false);
      return;
    }
    toast.success('Role-picker panel deleted');
    router.push(`/g/${guildId}/role-picker`);
    router.refresh();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Delete this role-picker panel?</DialogTitle>
          <DialogDescription>
        ...
        <ul className="space-y-2.5 text-sm">
          ...
          <li className="flex items-start gap-2">
            ...
            <span>
              ...
              <span className="text-[color:var(--color-fg-muted)]">
                Granted / revoked / failed events for this panel are deleted.
              </span>
            </span>
          </li>
        </ul>
