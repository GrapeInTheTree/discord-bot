...
import { countSelfRolesOptionHolders, removeSelfRolesOption } from '@/actions/self-roles-options';
...
export function RemoveSelfRolesOptionButton({
  ...
  React.useEffect(() => {
    ...
    void countSelfRolesOptionHolders({ guildId, panelId, optionId }).then((result) => {
      if (cancelled) return;
      setCounting(false);
  ...
  async function handleConfirm(): Promise<void> {
    ...
    setSubmitting(true);
    const result = await removeSelfRolesOption({
      guildId,
      panelId,
      optionId,
      cleanupRoles,
