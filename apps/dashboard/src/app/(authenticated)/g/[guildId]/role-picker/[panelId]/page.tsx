...
import { DeleteRolePickerButton } from '@/components/role-picker/delete-role-picker-button';
import { RemoveRolePickerOptionButton } from '@/components/role-picker/remove-role-picker-option-button';
import { RepostRolePickerButton } from '@/components/role-picker/repost-role-picker-button';
import { RetrySyncRolePickerButton } from '@/components/role-picker/retry-sync-role-picker-button';
import { RolePickerPreview } from '@/components/role-picker/role-picker-preview';
...
interface RolePickerDetailPageProps {
...
export default async function RolePickerDetailPage({
  params,
}: RolePickerDetailPageProps): Promise<React.JSX.Element> {
  ...
  const panel = await dbDrizzle.query.rolePickerPanel.findFirst({
    ...
    with: { options: { orderBy: asc(schema.rolePickerOption.position) } },
  });
      ...
      <Topbar
        ...
        action={
          <div className="flex items-center gap-2">
            {!isStale ? <RepostRolePickerButton guildId={guildId} panelId={panelId} /> : null}
            <Button asChild variant="secondary" size="sm">
              <Link href={`/g/${guildId}/role-picker/${panelId}/edit`}>Edit</Link>
            </Button>
          </div>
        }
      />
      <main className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-8 py-12">
        {isStale ? (
          <Card>
            ...
            <CardContent>
              <RetrySyncRolePickerButton guildId={guildId} panelId={panelId} />
            </CardContent>
          </Card>
        ) : null}
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between">
            <div className="flex flex-col gap-1.5">
              ...
              <CardDescription>
                Up to 25 options per panel (Discord StringSelectMenu cap). Each must have a unique
                label, role, and slot.
              </CardDescription>
            </div>
            <Button asChild size="sm" disabled={panel.options.length >= 25}>
              <Link href={`/g/${guildId}/role-picker/${panelId}/options/new`}>Add option</Link>
            </Button>
          </CardHeader>
          <CardContent>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1">
                        <Button asChild variant="ghost" size="sm">
                          <Link href={`/g/${guildId}/role-picker/${panelId}/options/${o.id}/edit`}>
                            Edit
                          </Link>
                        </Button>
                        <RemoveRolePickerOptionButton
                          guildId={guildId}
                          panelId={panelId}
                          optionId={o.id}
                          optionLabel={o.label}
        ...
        <Card destructive>
          ...
          <CardContent>
            <DeleteRolePickerButton guildId={guildId} panelId={panelId} />
          </CardContent>
        </Card>
      </main>
    </>
