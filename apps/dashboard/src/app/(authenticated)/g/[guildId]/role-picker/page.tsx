...
interface RolePickerListPageProps {
  readonly params: Promise<{ readonly guildId: string }>;
}

export default async function RolePickerListPage({
  params,
}: RolePickerListPageProps): Promise<React.JSX.Element> {
  const session = await auth();
  if (session === null) redirect('/login');

  const { guildId } = await params;
  const panels = await dbDrizzle.query.rolePickerPanel.findMany({
  ...
  return (
    <>
      <Topbar
        ...
        avatarUrl={avatarUrl}
        title="Role picker"
        description="StringSelectMenu panels — users open a dropdown and pick the option that grants their role."
        action={
          <Button asChild>
            <Link href={`/g/${guildId}/role-picker/new`}>New panel</Link>
          </Button>
        }
      />
        ...
        {panels.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
              <ListChecks
                ...
                aria-hidden="true"
              />
              <p className="text-base font-medium">No role-picker panels yet</p>
              <p className="max-w-sm text-sm text-[color:var(--color-fg-muted)]">
                Create a panel, bind options to roles, and post it to a channel. Members open the
                dropdown and pick the role they want — single-select per panel.
              </p>
              <Button asChild>
                <Link href={`/g/${guildId}/role-picker/new`}>Create your first panel</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
          <ul className="grid gap-3">
            {panels.map((p) => {
              ...
              return (
                <li key={p.id}>
                  <Link href={`/g/${guildId}/role-picker/${p.id}`} className="block">
