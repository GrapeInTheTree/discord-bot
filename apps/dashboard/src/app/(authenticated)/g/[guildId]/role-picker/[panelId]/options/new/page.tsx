...
interface NewRolePickerOptionPageProps {
  readonly params: Promise<{ readonly guildId: string; readonly panelId: string }>;
}

export default async function NewRolePickerOptionPage({
  params,
}: NewRolePickerOptionPageProps): Promise<React.JSX.Element> {
  const session = await auth();
  if (session === null) redirect('/login');
  const { guildId, panelId } = await params;

  const [panel, resources] = await Promise.all([
    dbDrizzle.query.rolePickerPanel.findFirst({
