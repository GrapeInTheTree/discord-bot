...
import { RolePickerOptionForm } from '@/components/role-picker/role-picker-option-form';
import { Card, CardContent } from '@/components/ui/card';
import { auth } from '@/lib/auth';
import { callBot } from '@/lib/botClient';
import type { GuildResources } from '@/types/bot';

interface EditRolePickerOptionPageProps {
...
export default async function EditRolePickerOptionPage({
  params,
}: EditRolePickerOptionPageProps): Promise<React.JSX.Element> {
  const session = await auth();
  if (session === null) redirect('/login');
  const { guildId, panelId, optionId } = await params;

  const [option, resources] = await Promise.all([
    dbDrizzle.query.rolePickerOption.findFirst({
